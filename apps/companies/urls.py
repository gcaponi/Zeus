from django.urls import path

from apps.companies import views

urlpatterns = [
    path("company/", views.company_detail, name="company-detail"),
    path("company/dna/", views.dna_current, name="dna-current"),
    path("company/dna/history/", views.dna_history, name="dna-history"),
    path("company/dna/create/", views.dna_create, name="dna-create"),
    path("company/dna/generate/", views.dna_generate, name="dna-generate"),
    path("company/dna/questions/", views.dna_questions, name="dna-questions"),
    path(
        "company/dna/gap-questions/<int:round_number>/",
        views.dna_gap_questions,
        name="dna-gap-questions",
    ),
    path(
        "company/dna/processing/<str:operation>/<int:round_number>/",
        views.dna_processing,
        name="dna-processing",
    ),
    path("company/dna/<int:pk>/feedback/", views.dna_feedback, name="dna-feedback"),
    path("company/dna/review/", views.dna_review, name="dna-review"),
    path(
        "company/dna/specialists/analyze/",
        views.dna_cross_specialist_analyze,
        name="dna-cross-specialist-analyze",
    ),
    path(
        "company/dna/specialists/apply/",
        views.dna_cross_specialist_apply,
        name="dna-cross-specialist-apply",
    ),
    path("company/dna/motore-b/", views.motore_b_report, name="motore-b-report"),
    path("company/dna/consistency/", views.consistency_report, name="consistency-report"),
    path("company/dna/consistency/run/", views.consistency_audit_run, name="consistency-audit-run"),
    path(
        "company/dna/consistency/<int:pk>/<str:action>/",
        views.consistency_issue_action,
        name="consistency-issue-action",
    ),
    path("company/dna/visualize/", views.dna_visualize, name="dna-visualize"),
    path("company/dna/generating/", views.dna_generating, name="dna-generating"),
    path("company/dna/download/", views.dna_download_pdf, name="dna-download-pdf"),
    path(
        "company/dna/<int:pk>/section/<str:section_key>/approve/",
        views.dna_section_approve,
        name="dna-section-approve",
    ),
    path(
        "company/dna/<int:pk>/section/<str:section_key>/edit/",
        views.dna_section_edit,
        name="dna-section-edit",
    ),
    path("products/", views.product_list_create, name="specialista-list-create"),
    path("products/<int:pk>", views.product_detail),
    path("products/<int:pk>/", views.product_detail, name="specialista-detail"),
    path("products/<int:pk>/upload/", views.product_file_upload, name="specialista-file-upload"),
    path(
        "products/<int:pk>/files/<int:file_pk>/delete/",
        views.product_file_delete,
        name="specialista-file-delete",
    ),
    path("products/<int:pk>/delete/", views.product_delete, name="specialista-delete"),
    path("products/<int:pk>/generate/", views.product_dna_generate, name="specialista-dna-generate"),
    path("products/<int:pk>/questions/", views.product_questions, name="specialista-questions"),
    path(
        "products/<int:pk>/gap-questions/<int:round_number>/",
        views.product_gap_questions,
        name="specialista-gap-questions",
    ),
    path(
        "products/<int:pk>/gap-questions/<int:round_number>/processing/",
        views.product_gap_processing,
        name="specialista-gap-processing",
    ),
    path("products/<int:pk>/review/", views.product_review, name="specialista-review"),
    path("products/<int:pk>/promote/", views.product_promote, name="specialista-promote"),
    path("products/<int:pk>/publish/", views.product_publish, name="specialista-publish"),
    path(
        "products/<int:pk>/publications/<int:publication_pk>/archive/",
        views.product_publication_archive,
        name="specialista-publication-archive",
    ),
    path(
        "products/<int:pk>/consistency/check/",
        views.product_consistency_check,
        name="specialista-consistency-check",
    ),
    path("products/<int:pk>/critique/<int:proposal_index>/accept/", views.product_critique_accept, name="specialista-critique-accept"),
    path("products/<int:pk>/critique/<int:proposal_index>/reject/", views.product_critique_reject, name="specialista-critique-reject"),
    path("products/<int:pk>/visualize/", views.product_dna_visualize, name="specialista-dna-visualize"),
    path("products/<int:pk>/download/", views.product_dna_download_pdf, name="specialista-dna-download-pdf"),
    path("products/<int:pk>/feedback/", views.product_dna_feedback, name="specialista-dna-feedback"),
    path(
        "products/<int:pk>/feedback/apply/",
        views.product_dna_feedback_apply,
        name="specialista-dna-feedback-apply",
    ),
    path(
        "products/<int:pk>/section/<str:section_key>/approve/",
        views.product_section_approve,
        name="specialista-section-approve",
    ),
    path(
        "products/<int:pk>/section/<str:section_key>/edit/",
        views.product_section_edit,
        name="specialista-section-edit",
    ),
    path("sources/", views.source_list_create, name="source-list-create"),
    path("agent/", views.agent_chat, name="agent-chat"),
    path("agent/send/", views.agent_send, name="agent-send"),
    path("agent/clear/", views.agent_clear, name="agent-clear"),
    path("guide/send/", views.guide_send, name="guide-send"),
    path("guide/clear/", views.guide_clear, name="guide-clear"),
    path("guide/state/", views.guide_state, name="guide-state"),
    path("sources/<int:pk>/", views.source_detail, name="source-detail"),
    path("pipeline/", views.pipeline_run_create, name="pipeline-run-create"),
    path("pipeline/<int:pk>/", views.pipeline_run_detail, name="pipeline-run-detail"),
    path("generation-progress/<int:pk>/", views.generation_progress, name="generation-progress"),
]
