...
- Initial Company Foundation questions are generated by `generate_company_questions_task`
  instead of blocking `dna_questions` inside the HTTP request.
- Foundation and follow-up answers are evaluated by `process_company_gap_round_task`; the task
  creates the next Gap round or dispatches complete DNA generation with tenant context preserved.
- A shared App Shell and legacy processing page polls both operations with HTMX and redirects to
  the questionnaire, next Gap round, or complete-DNA generation when the worker finishes.
